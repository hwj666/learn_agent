...
class DynamicPlanExecuteAgent(BaseAgent):
    ...
    async def run(self, user_query: str) -> str:
        ...
        while step < self.max_steps:
            step += 1
            logger.info(f"==================== plan 步骤 {step}/{self.max_steps} ====================")

            # ------------------ 阶段 A：滚动规划与决策 ------------------
            decision, resp = await self._stage_a_plan(user_query, todo_list_summary, execution_history, last_result)

            # 强防御性结束拦截
            is_empty_tool_calls = resp.tool_calls is None or len(resp.tool_calls) == 0
            if "[FINISH]" in decision and is_empty_tool_calls:
                exit_msg, should_continue = self._handle_finish_intercept(has_called_tool, has_success)
                if should_continue:
            ...
            if resp.tool_calls and len(resp.tool_calls) > 0:
                ...
                if continuous_todo_count > 3:
                    logger.warning("🚨 触发架构熔断：模型陷入连续维护 TODO 清单死循环，强行拦截。")
                    last_result = "系统警告：检测到你连续多次在原地更新待办清单，请立刻停止调用 manager_todo！请直接输出纯文本宣告下一步的具体业务行动。"
                    continuous_todo_count = 0  
                    continue
                
                # 执行清单维护并切断通路，直接进入下一轮循环
                todo_list_summary, last_result = await self._execute_todo_tool(resp.tool_calls, step, execution_history)
                continue 
            ...
            logger.info(f"==================== exec 步骤 {step}/{self.max_steps} ====================")
            # ------------------ 阶段 B：业务参数提取 (正则兜底) ------------------
            final_tool_calls = await self._stage_b_extract_parameters(user_query, decision, last_result)
            
            # 提取失败拦截
            if len(final_tool_calls) == 0:
                last_result = f"错误：工具引擎未能将动作 [{decision}] 转化为有效的业务 Tool Call。请在下一轮规划中给出更清晰的指令。"
                execution_history.append(f"步骤 {step} 异常: 动作无法转化为业务 Tool Call。")
                continue

            # ------------------ 阶段 C：执行业务工具并获取返回 ------------------
            has_success, output_text = await self._stage_c_execute_business_tools(final_tool_calls)
            has_called_tool = True
    ...
    async def _stage_a_plan(self, user_query: str, todo_summary: str, history: List[str], last_res: str) -> Tuple[str, Any]:
        """阶段 A：获取规划层 LLM 决策结果"""
        ...
        planner_msgs = [LLMMessage.system(self.plan_system), LLMMessage.user(plan_prompt)]
        resp = await self.client.chat(messages=planner_msgs, tools=[self.executor.get_tool("manager_todo")])
        decision = resp.content.strip() if resp.content else ""
        return decision, resp

    def _handle_finish_intercept(self, has_called_tool: bool, has_success: bool) -> Tuple[str, bool]:
        """校验 [FINISH] 状态，防止模型未做工作或带着错误摆烂退出"""
        if not has_called_tool:
            return "错误：检测到你试图在未执行任何工具前结束任务。请先调用工具拆解或执行任务。", True
        if not has_success:
            return "错误：任务未成功完成（历史步骤存在未解决的错误），请给出修复/调试动作继续执行。", True
        logger.info(f"✅ 任务顺利完成退出")
        return "任务已成功通过 TODO 清单动态拆解并执行完毕。", False

    async def _execute_todo_tool(self, tool_calls: List[Any], step: int, history: List[str]) -> Tuple[str, str]:
        """执行本地的 TodoTool 任务管理工具"""
        logger.info(f"📋 检测到 TodoTool 调用，启动框架层本地执行拦截。")
        try:
            results = await self.executor.execute(tool_calls=tool_calls, ctx=self.ctx, timeout=30)
    ...
    def _parse_todo_summary(self, tool_output: str) -> str:
        ...
        try:
            ...
            if res_data.get("success"):
                ...
                lines = [f"- [Index: {t['index']}] {t['title']} (状态: {t['status']})" for t in todos]
    ...
    async def _stage_b_extract_parameters(self, user_query: str, decision: str, last_result: str) -> List[Dict[str, Any]]:
        ...
        llm_result = await self.client.chat(messages=exec_msgs, tools=self.executor.tools)
        
        final_tool_calls = llm_result.tool_calls or []
        return final_tool_calls
        
    async def _stage_c_execute_business_tools(self, tool_calls: List[Any]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if any(keyword in output_text.lower() for keyword in ["错误", "fail", "error"]):
