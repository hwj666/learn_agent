...
class ReactAgent(BaseAgent):
    async def run(self, user_query: str) -> str:
        ...
        while step < self.max_steps:
            ...
            try:
                llm_response = await self.client.chat(messages=self.memory, tools=self.executor.tools)
                print("\033[0m")
            except Exception as e:
                return f"错误：调用模型时发生异常: {str(e)}"

            assistant_msg = LLMMessage.assistant(
                content=llm_response.content or "", tool_calls=llm_response.tool_calls or None
            )
            ...
            tool_responses = await self.executor.execute(tool_calls=llm_response.tool_calls, ctx=self.ctx, timeout=30.0)
