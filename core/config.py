...
class AppConfig:
    ...
    def __post_init__(self):
        ...
        for agent_name, agent_cfg in self.agents.items():
            if agent_cfg.model not in self.models:
                raise ValueError(f"智能体 {agent_name} 引用了不存在的模型 {agent_cfg.model}")
    ...
    def from_yaml(cls, file_path: str) -> "AppConfig":
        ...
        return cls(
            agents={name: AgentConfig(**cfg) for name, cfg in data.get("agents", {}).items()},
            model_providers={name: ProviderConfig(**cfg) for name, cfg in data.get("model_providers", {}).items()},
            models={name: ModelConfig(**cfg) for name, cfg in data.get("models", {}).items()},
        )
