...
class ToolExecutor:
    ...
    async def execute(
        ...
        for i, res in enumerate(results):
            ...
            if isinstance(res, Exception):
                logger.error(
                    f"💥 工具调度层严重崩溃 [{tool_call.name}]: {res}", exc_info=True
                )
                tool_result = ToolResult(
                    success=False, content=f"系统调度层异常: {str(res)}", error=str(res)
                )
            # 2. 标准 ToolResult 响应
            ...
            # 3. 严格拒绝任何非标返回（防止脏数据污染大模型）
                ...
                logger.error(
                    f"❌ 工具 [{tool_call.name}] 返回了不合规的数据类型: {type(res)}"
                )
                tool_result = ToolResult(
                    success=False,
            ...
            logger.info(
                f"📊 [工具报表] 名称: {tool_call.name} | 状态: {status} | 结果: {tool_result.content}"
            )
    ...
    async def _execute_single(
        ...
        if not tool_cls or tool_cls.toolset not in self.allowed_toolsets:
            return ToolResult(
                success=False,
                content=f"❌ 权限拒绝：当前 Agent 未获准使用工具 `{tool_call.name}`",
            )
        ...
        async with self._semaphore:
            try:
                ...
                if has_state:
                    # 使用封装好的安全异步锁上下文
                    async with self._use_tool_lock(
                        session_id, agent_id, tool_call.name
                    ):
                        logger.info(
                            f"🔒 [串行加锁区] 正在加载状态数据: {tool_call.name}"
                        )
                        raw_state = await self.storage.get_state(state_key)

                        # 在锁安全的保护伞内实例化并注入专属影子状态
                        shadow_ctx["tool_state"] = (
                            tool_cls.state_schema(**raw_state)
                            if raw_state
                            else tool_cls.state_schema()
                        )

                        logger.info(f"🚀 [有状态通道] 启动串行调用 -> {tool_call.name}")
                        result = await self._execute_core_with_timeout(
                            tool_instance, shadow_ctx, validated_args, timeout
                        )

                        # 执行成功立即刷回底层存储（如 Redis/Memory），随后安全释放锁
                        await self.storage.set_state(
                            state_key, shadow_ctx["tool_state"].model_dump()
                        )
                else:
                ...
                return ToolResult(success=True, content=str(content_str))

            except ValidationError as e:
                return ToolResult(
                    success=False, content=f"错误：参数不匹配大模型契约 → {str(e)}"
                )
            except asyncio.TimeoutError:
                return ToolResult(
                    success=False, content=f"错误：该工具执行已超时（阈值 {timeout}s）"
                )
            except Exception as e:
                logger.exception(f"❌ 工具 {tool_call.name} 运行期间抛出业务异常")
                return ToolResult(success=False, content=f"错误：{str(e)}")
