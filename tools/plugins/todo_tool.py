...
class TodoToolArgs(BaseModel):
    ...
    title: Optional[str] = Field(
        default=None,
        description="新的单个待办标题（仅在 update 修改单个标题时使用，或作为 add 时的单条兜底）",
    )
    index: Optional[int] = Field(
        default=None,
        description="从1开始的序号（修改 update、删除 delete 时必填，由列表返回）",
    )
    status: str = Field(
        default="pending",
        ...
        return json.dumps(
            {
                ...
                "summary": {
                    ...
                    "pending": sum(
                        1 for x in state.items if x.get("status") == "pending"
                    ),
                    "in_progress": sum(
                        1 for x in state.items if x.get("status") == "in_progress"
                    ),
                    "completed": sum(
                        1 for x in state.items if x.get("status") == "completed"
                    ),
                    "cancelled": sum(
                        1 for x in state.items if x.get("status") == "cancelled"
                    ),
                },
            },
            ensure_ascii=False,
