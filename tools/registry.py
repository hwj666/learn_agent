...
class ToolRegistry:
    ...
    @classmethod
    def register(cls, *, name: str, toolset: str) -> Callable[[Type[BaseTool]], Type[BaseTool]]:
        """
            ...
            with cls._register_lock:
                if name in cls._registry:
                    ...
                    if existing_cls is not tool_cls:
                        ...
                        is_same_file = (file_existing and file_current and file_existing == file_current)
                        is_same_classname = existing_cls.__qualname__ == tool_cls.__qualname__
                        
                        if is_same_file and is_same_classname:
                            cls._registry[name] = tool_cls
                            print(f"🔄 幂等更新工具：{name} (由于不同的导入路径重复加载)")
                            return tool_cls
