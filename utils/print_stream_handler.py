...
class PrintStreamHandler:
    ...
    async def __aenter__(self):
        ...
        print(f"{self.COLOR_SYSTEM}🤖 [系统] 正在连接大模型并建立流式通道...{self.COLOR_RESET}", flush=True)
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        print(f"\n\n{self.COLOR_SYSTEM}🏁 [系统] 模型流式传输结束。\n{self.COLOR_RESET}", flush=True)
        return False

    async def __call__(self, think: str, text: str, cleaned_tool_deltas: List[Dict], chunk_type: str):
        if not chunk_type:
            return
        ...
        if self.last_chunk_type == "thinking" and think:
            ...
            sys.stdout.write(f"{self.COLOR_GREY}{self.COLOR_DIM}{payload}{self.COLOR_RESET}")
            sys.stdout.flush()
